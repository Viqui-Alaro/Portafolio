...
// --- 1. COMPONENTE DE CARGA (Con Cuadrados Flotantes y Letra por Letra) ---
const Preloader = ({ isFading }) => {
  // Separamos la palabra en un arreglo de letras para animarlas una por una
  ...
  return (
    <div style={{
      ...
      width: "100vw",
      height: "100vh",
      backgroundColor: "#0a192f", /* Un azul marino más profundo y tecnológico */
      display: "flex",
      ...
      overflow: "hidden" /* Evita que los cuadrados generen scroll */
    }}>
      
      {/* --- FONDO ANIMADO DE CUADRADOS FLOTANTES --- */}
      <div className="squares-container">
        {[...Array(10)].map((_, i) => (
          <div key={i} className="floating-square" style={{
            left: `${Math.random() * 100}%`, /* Posición horizontal aleatoria */
            animationDelay: `${Math.random() * 5}s`, /* Inician en momentos distintos */
            animationDuration: `${5 + Math.random() * 5}s`, /* Velocidades distintas */
            width: `${20 + Math.random() * 40}px`, /* Tamaños distintos */
            height: `${20 + Math.random() * 40}px`
          }}></div>
        ))}
      </div>

      {/* --- TEXTO "LOADING..." LETRA POR LETRA --- */}
        ...
        {loadingText.map((char, index) => (
          <span 
            ...
            style={{
              ...
              animation: "appearLetter 0.5s forwards",
              animationDelay: `${index * 0.15}s` /* Cada letra tarda 0.15s más que la anterior */
            }}
          >
            {char}
          </span>
      ...
      {/* --- LAS REGLAS DE ANIMACIÓN CSS (Keyframes) --- */}
      <style>{`
        /* Animación para que las letras aparezcan y reboten un poco */
        @keyframes appearLetter {
          0% { opacity: 0; transform: translateY(20px); }
          50% { opacity: 1; transform: translateY(-5px); text-shadow: 0 0 20px #00f0ff; }
          100% { opacity: 1; transform: translateY(0); }
        }

        /* Animación para los cuadrados de fondo */
        .floating-square {
          position: absolute;
          bottom: -100px;
          background: rgba(0, 240, 255, 0.05); /* Cyan muy transparente */
        ...
        @keyframes floatUp {
          ...
          100% { transform: translateY(-100vh) rotate(720deg); opacity: 0; border-radius: 50%; }
        }
      `}</style>
    </div>
  );
...
// --- 2. Animación de escritura nativa del portafolio ---
const TypewriterEffect = () => {
  const texts = [
    "Full-Stack Developer",
    "Especialista en Java & Spring Boot",
    "Creadora de proyectos Open Source"
  ];
...
export default function HeroV2({ t, lang }) {
  ...
  useEffect(() => {
    // Aumenté el tiempo a 2.5s para que se aprecie mejor la animación de las letras
